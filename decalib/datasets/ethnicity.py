...
"""
顔画像データセットに対して前処理を行い、画像や顔のランドマーク、セグメンテーションマスクを
Pytorchのテンソル形式で返すクラスを実装
"""
...
class EthnicityDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.K):
            ...
            # 画像とランドマーク、セグメンテーションマスクを取得                
            image = imread(image_path)/255.
            kpt = np.load(kpt_path)[:,:2]
            mask = self.load_mask(seg_path, image.shape[0], image.shape[1])

            ### crop information
            tform = self.crop(image, kpt)   # 画像のクロップ情報取得
            ## crop 
            # warp関数で指定サイズにクロップ・リサイズした画像、マスク、ランドマーク座標を作成
    ...
    # バウンディングボックス情報からcropに必要な変換行列を提供
    ...
    def load_mask(self, maskpath, h, w):
        ...
        if os.path.isfile(maskpath):
            ...
            # for i in range(1, 16):
            # マスクの画素値が0.5以上の部分を1、未満の部分を0に設定して二値化
            mask[vis_parsing_anno>0.5] = 1.
        else:
            # マスクが存在しない場合は、全ての値が1のマスクを生成
            mask = np.ones((h, w))
        return mask
