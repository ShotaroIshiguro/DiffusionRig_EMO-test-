...
def build_dataloader(config, is_train=True):
    ...
    # configに含まれるデータセットの種類に応じて、各データセットを初期化し、data_listに追加
...
# CelebA-HQデータセットから顔画像を読み込み、顔のクロップとリサイズ処理を行う
class CelebAHQDataset(Dataset):
    ...
    def __getitem__(self, idx):
        while(100):
            ...
            # kptnameから画像とキーポイントのファイル名を作成し、それぞれのパスを構築
...
# テスト画像または動画を読み込み、顔の検出とクロップ処理を行う   
...
# 評価データセットの画像やビデオを読み込み、クロップやリサイズなどの前処理を行う
