...
"""
画像から3D顔モデルを再構築し、ジオメトリやテクスチャ情報を生成する
ResNetを使ったエンコーダ、FLAME3D顔モデル、UVマッピングによる顔のディテール処理、レンダリング、3Dモデルの出力を行う
"""
...
class DECA(nn.Module):
    ...
    def _setup_renderer(self, model_cfg):
        ...
        # SRenderY：顔の形状・テクスチャ・照明条件から2D画像として顔をレンダリング
    ...
    def _create_model(self, model_cfg):
        ...
        # encoders
        # 画像から形状とディテールをエンコードするためのResNetベースのエンコーダ
        self.E_flame = ResnetEncoder(outsize=self.n_param).to(self.device) 
        self.E_detail = ResnetEncoder(outsize=self.n_detail).to(self.device)
        # decoders
        # 3D顔形状モデルを表現するFLAMEで、形状・表情・姿勢パラメータを生成
        self.flame = FLAME(model_cfg).to(self.device)
        if model_cfg.use_tex:
            # 顔の表面に色や質感をマッピングするためのモデル
            self.flametex = FLAMETex(model_cfg).to(self.device)
        # ディテール情報を出コードすることで、詳細な表面の形状を生成
    ...
    # 画像からのエンコード出力を形状・表情・姿勢・カメラ・ライトの各パラメータに分解
    ...
    # 詳細情報を含むUVディスプレースメントマップから、3Dモデルの表面法線ベクトルを示すノーマルマップを生成
    ...
    # 顔のランドマークがカメラから視認かどうかを法線ベクトルのカメラに対するz成分から判定
    ...
    # E_flameを使用して、入力画像から形状や表情、姿勢パラメータをエンコード
    # エンコードされたパラメータは、decompse_codeで分解され、codedictとして返される
    # @torch.no_grad()
    ...
    # FLAMEモデルを使って、エンコードされたパラメータから3Dの頂点座標と2D/3Dランドマークを再構成
    # テクスチャはFLAMEtexで生成、2D画像としてレンダリングも可能
    # opdictには頂点、ランドマーク、レンダリング結果が格納
    # @torch.no_grad()
    ...
    # 入力画像のパスを受け取り、3D顔モデルの再構成・レンダリングを行う
    # 入力画像カラエンコードされたパラメータcodedictを取得し、それを利用したデコード結果opdict(再構成結果), visdict(可視化用のレンダリング結果)返す
