...
"""
顔形状を生成するFLAMEクラスと、テクスチャ情報を生成するFLAMETexクラス
"""
...
# scipy.sparseの疎行列である場合、通常のnp配列に変換
def to_np(array, dtype=np.float32):
    if 'scipy.sparse' in str(type(array)):
        array = array.todense()
    return np.array(array, dtype=dtype)

# dictionary形式のデータを属性として保持
# FLAMEのも出るデータを読み込む際に使用
class Struct(object):
...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        # flame_model_pathからFLAMEモデルパラメータを読み込み、Structオブジェクトとして保持
    ...
    # 頭の相対的な向きに基づいて、顔の輪郭に関連する動的ランドマークのインデックスと座標を取得
    ...
    # メッシュの頂点位置からバリセントリック補間によりランドマークを計算
    ...
    # 形状・表情・姿勢パラメータを受け取り、メッシュとランドマークを生成
...
# 顔のテクスチャ情報を管理
# FLAME固有のテクスチャ空間を読み込み、顔画像を再現するためのテクスチャを生成
class FLAMETex(nn.Module):
    ...
    """
    # テクスチャデータをtensorに変換し、バッファとして塘路億
    def __init__(self, config):
