...
"""
DECAを用いたトレーニングループの実装 CoarseとDetailモードを分けて実装
Detailモードでは、エンコーダだけでなくデコーダも最適化対象
"""
...
class Trainer(object):
    ...
    # CoarseかDetailかによって、トレーニングするパラメータが異なる
    ...
    # resumeオプションが有効な場合は、以前保存されたモデルからトレーニングを再開
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        # 入力としてbatch(画像データとラベル)を受け取り、self.deca.encodeでエンコードし、形状・表情などのパラメータ取得
        #-- encoder
        ...
        # Coarseモデルのトレーニングでは、エンコードされたパラメータを使い、decodeで3D顔を再構成する
        # decodeデータから、2D画像再構成や2Dランドマークの予測結果を生成
        ...
        # Detailモデルのトレーニングでは、顔の詳細を含むディスプレースメントマップを生成
        # ディスプレースメントマップから法線マップを計算し、ディテールのシェーディング追加
        else:
            #-- decoder
            shapecode = codedict['shape']
        ...
        #########################################################
        all_loss = 0.
        # coarse・detailそれぞれのモードにおいて、計算した全ての損失を足し合わせる
    ...
    # 検証用データセットを用いてencode, decodeを行い、結果を保存
    ...
    # NoWデータセットを用いたモデルの評価
    # 評価結果として再構成されたメッシュやランドマークの位置を保存
    def evaluate(self):
        ''' NOW validation 
        '''
