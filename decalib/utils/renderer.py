...
"""
3Dモデルの頂点や面・アルベドをもとに、画像上にモデルをレンダリングする処理を行う
異なる照明条件も追加可能
"""

# 標準的なnot微分可能なラスタライザを実装
...
# 入力として3D頂点、面、属性（例えば色や法線など）を受け取り、これらを2D画像上にラスタ化
class StandardRasterizer(nn.Module):
    ...
    # 頂点座標を画像空間に変換
    # ラスタライザを使って、深度バッファ、三角形バッファ、バリセントリック座標を計算
    # 以上の利用し、ピクセルの値を計算し、2D画像を生成
...
class Pytorch3dRasterizer(nn.Module):
    ...
    # PyTorch3Dの rasterize_meshes 関数を使用して、入力された3Dメッシュを2D画像に変換
...
# レンダリング全体を管理するクラスで、頂点、アルベド（反射率マップ）、照明設定を基に3Dオブジェクトを画像にレンダリング
# pytorch3d由来かnot微分可能ラスタライザを用いるかを選択可能
class SRenderY(nn.Module):
    ...
    # 3D頂点と変換後の頂点を基に、法線や頂点の情報を計算し、それを元にラスタライザで画像を生成
    # オプションで背景画像やライトを追加してレンダリング結果を生成
    ...
    # ワールド空間の頂点をUV空間に変換
    # UVマッピング(3D-2Dの対応)を使用したレンダリングが可能
