...
"""
3D回転表現を変換するための関数を提供
オイラー角 (Euler Angles): 3つの回転軸に対する角度で3D回転を表現する。
回転行列 (Rotation Matrix): 3x3の行列で3D回転を表現する。
角度軸 (Angle Axis): 回転の方向と回転角度を持つベクトルで3D回転を表現する。
クォータニオン (Quaternion): 4次元ベクトルで回転を表現し、回転行列の代替となる形式。
"""
...
######### to quaternion
# オイラー角(x, y, z)からクォータニオン(w, x, y, z)に変換
