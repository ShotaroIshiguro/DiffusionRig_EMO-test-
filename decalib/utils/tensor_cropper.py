...
"""
画像から特定の領域をクロップし、その領域内で座標の位置を適切に変換
korniaライブラリのジオメトリ変換機能も提供

コードの流れ
1. points2bboxで点群のバウンディングボックスを計算
2. augment_bboxをバウンディングボックスをランダムに拡張・平行移動
3. crop_tensorでバウンディングボックスに基づいて画像をクロップし、点群をクロップ後の座標系に変換するための変換行列を計算
4. 元の点群をクロップ後の座標系に変換
"""

# 入力として点群を与えると、その点群に最適なバウンディングボックスを計算
...
# 既存バウンディングボックスにランダムなスケールと平行移動操作を適用
...
# 画像を指定されたバウンディングボックスでクロップ
# warp_affineで、指定領域を正方形のクロップサイズにリサイズしながらクロップ、点群の変換行列も提供
...
# 画像と点群を受け取り、クロップされた画像と変換行列を返す
# 画像をクロップした後、元の座標をクロップ後の画像に合わせて変換
