...
"""
DECAの一部であり、3D顔形状の再構成とテクスチャ、表情の推定を行うために使用
入力された顔画像から3Dモデルを推定し、トレーニングデータと比較して損失を計算し、学習を進める
"""
...
class Trainer(object):
    ...
    # エンコーダ(E_flame)のパラメータを対象にしたAdamオプティマイザーを設定
    ...
    def training_step(self, batch, batch_nb):
        ...
        # バッチとして入力された画像、ランドマーク、マスクを変換して使用
        ...
        #-- encoder
        # 画像をエンコードし、3D形状や表情、テクスチャを推定
        codedict = self.deca.encode(images)
        
        ### shape constraints
        # 3D形状推定の際に異なる入力画像が同じ形状を持つように学習を促すためのデータ拡張
    ...
    # ランドマーク損失、identity損失、ジオメトリや表情の正則化項などの損失を返す
    ...
    # NoWDatasetを使って、3D再構成の精度を確認
    # メッシュ(.obj)やランドマーク座標などを保存
    def evaluate(self):
        ''' NOW validation 
        '''
        ...
        for epoch in tqdm(range(start_epoch, self.cfg.train.max_epochs)):
            for step, batch in enumerate(tqdm(self.train_dataloader)):
                ...
                if self.global_step % self.cfg.train.eval_steps == 0:
                    self.evaluate()
                # 損失のバックプロパゲーションとオプティマイザーの更新(エンコーダのみ)
