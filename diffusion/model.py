...
# 2次元空間データに対して注意機構を適用する
# 空間次元を減らしつつ、入力データの重要な特徴を保持できる
# 入力データに位置embeddingを適用し、QKVAttentionによる注意機構の計算を行う
...
# タイムステップ埋め込みやdetail_condのサブモジュール化
...
# 残差ブロックの定義
# タイムステップ埋め込みやdetail_condが渡されると、それに応じてスケールシフト正規化を行い、特徴量を変換
# 画像の解像度変換も可能
...
# Q, K, Vの3つの特徴量ベクトルを使って、Attention重みを計算し、異なる空間位置間の依存関係を学習
...
# ダウンサンプリング(encoder)とアップサンプリング(decoder)から構成されるU-Netを定義
# 各レベルで残差ブロックと注意機構を適用し、タイムステップや物理条件を考慮した特徴量変換を行う
...
# U-Netとともに、Global Latent Codeをエンコードする機構を持つ拡散モデル
class DiffusionRig(nn.Module):
    ...
    def forward(self, x, t, physic_cond, detail_cond=None, x_start=None):
        ...
        losses = {}
        # x_start(画像などの入力データ)をResNet18or50でエンコードすることで、物理条件とは異なるグローバル潜在コード(detail_cond)を生成
        if detail_cond is None:
            detail_cond = self.encode_cond(x_start)

        # 画像と物理条件の結合を行ったデータと一緒にGlobal latent codeをU-Netのデコーダに入力することでノイズ予測
        output = self.denoisingUNet(x, t, physic_cond, detail_cond)

        # Diffusionrigの出力outputにノイズ除去後の画像とからの辞書である損失を格納
        return output, losses

    def encode_cond(self, x):
        return self.encoder(x)

# 画像サイズやチャネル数、残差ブロック数の数、アテンションレイヤーなどの設定に基づいてUnetModelを構築
def create_model(
    image_size,
    num_channels,
    num_res_blocks,
