...
"""
DECAモデルを使い、ソース画像の3D顔形状や表情を別のターゲット画像に近づけるように最適化を行う
EmoNetを使用し、感情や表情の特徴を利用して損失を計算することで、ターゲット画像に近い感情表現を生成
"""
...
# 入力画像を読み込み、モデルが処理可能なバッチ形式に変換
# ランドマークやセグメンテーション情報も読み込む
def load_image_to_batch(image):
    # 画像・ランドマーク・セグメンテーションを格納する空の辞書batchを用意
    batch = {}
    # 入力が画像パスの場合は、ファイルを読み込み、RGBチャネルを選択
    if isinstance(image, str) or isinstance(image, Path):
        ...
        if 'detections' in str(image_path):
            ...
            # ランドマークファイルのパスを生成.pkl形式
            lmk_path = str(lmk_path).replace('.png', ".pkl")
            # 存在する場合は画像サイズで正規化し、[-1, 1]にスケーリング
    ...
    # 画像ファイルがnumpy形式の場合、チャネル順序変更、バッチ次元追加、[0, 1]にスケーリング
    ...
    # 画像・ランドマーク・セグメンテーションをバッチに格納し、GPUに転送
...
# ターゲット画像に基づいた感情損失を計算するためのモジュール
# EmoNetからの感情特徴量や感情スコア（バランス、興奮度、表情分類）から損失を計算
class TargetEmotionCriterion(torch.nn.Module):

    def __init__(self,
                 target_image,                  # 感情的特徴の基準となるターゲット画像
                 # EmoNetの感情特徴マップ、valence、arousal、感情分類を損失に含めるかどうか
                 use_feat_1 = False,
                 use_feat_2 = True,
                 use_valence = False,
                 use_arousal = False,
    ...
    # 入力画像に対して感情的特徴のそんしつを実行、実際の処理はcompute内
...
# DECAモデルで計算された損失項目を利用するためのモジュール
class DecaTermCriterion(torch.nn.Module):
    ...
    # DECAが出力する損失辞書loss_dictの中からkeywordに対応する損失値を返す
    def forward(self, loss_dict):
        return loss_dict[self.keyword]

# 入力回転ベクトルを回転形式に変換
def convert_rotation(input, rot_type):
    # 軸角表現、クォータにオン、オイラー角の三つの回転形式に対応
    if rot_type == "aa":
...
# 特定の顎回転jaw_poseと現在の顎回転の差を計算し、それを損失として扱う
# DECAの一部として、顎の動きをターゲット画像のposeに最適化するために使用
class TargetJawCriterion(torch.nn.Module):

    def __init__(self,
                 reference_pose,
                 reference_type,    # 顎回転形式
                 loss_type="l1"
                 ):
        super().__init__()
...
# 画像生成プロセスの中間および最終結果を比較、可視化するための関数
# 入力画像・粗形状・詳細形状・粗画像・詳細画像
...
# vis_dict(可視化辞書)から入力画像や形状情報、出力画像を抽出
def extract_images(vis_dict, detail=True):
    # キー名に含まれるdetai__inputsの手前までを接頭辞prefixとして切り出す
    prefix = None
    ...
    # detailが指定された場合は、詳細な顔形状データgeom_detail、詳細出力画像のキーを取り出す
    if detail:
...
# DECAモデルの最適化プロセス中に生成される3D顔の形状や出力画像を可視化し、保存するための関数
...
# 最適化プロセス中に記録された損失の履歴や、損失項目ごとの変化を可視化するための関数
...
# 最適化過程の進捗をフレームごとに記録し、それを元に動画を作成する関数
...
# DECAの潜在コード(形状・表情・姿勢)を最適化するための関数
def optimize(deca,
             values,                    # 最適化の初期値となるパラメータセット
             # optimize_detail=True,
             optimize_detail=False,
             optimize_identity=False,
             optimize_expression=True,  # 表情コードのみが最適化対象
    ...
    # 表情コードをtorch.autograd.Variableとして初期化し、最適化に含めるパラメータリストに追加
...
# 入力とターゲットを統合し、最適化の初期値を設定
# 入力画像とターゲット画像から潜在コードをコピーして初期値を構成
...
# DECAを用いてソース画像からターゲット画像に近づける形状や感情表現の最適化を実行
# 指定されたパラメータや損失関数に基づき最適化プロセスを実行し、結果を保存
def single_optimization(path_to_models, relative_to_path, replace_root_path, out_folder, model_name,
    ...
    initializations = {}
    # ソース画像の情報をモデルに通し、初期形状・表情・ポーズなどの潜在コードと可視化用のデータを取得
    ...
    # バッチ形式のターゲット画像をモデルに通し、初期値を取得
    values_target, visdict_target = test(deca, dm, batch=batch)
    # ソース画像とターゲット画像のコードを組み合わせ、最適化の初期状態を設定
    ...
    for key, vals in initializations.items():
        ...
        for i in range(num_repeats):
            ...
            # DECAモデルの各コードをターゲット画像に近づけるように最適化
            optimize(deca,
...
# ソース画像とターゲット画像に基づいて最適化プロセスを実行するエントリーポイント
def single_optimization_v2(path_to_models, relative_to_path, replace_root_path, out_folder, model_name,
    ...
    # input(ソース画像)に表情や姿勢のコードがターゲット画像から引き継がれる
    # 複数の初期化セット(異なるコード設定)が生成され、各セットで別々に最適化が行われる
...
# 複数のターゲット画像に対してDECAモデルの感情最適化プロセスを実行するためのループ処理を提供
...
def main():
    ...
    # deca_modelsに登録されているすべてのモデルについて、ターゲット画像を用いた最適化を実行
    for name, cfg in deca_models.items():
        model_folder = cfg[0]
        stage = cfg[1]
        starting_image_index = cfg[2]
        # 特定のターゲット画像に対して異なる損失項目を使った最適化を実行
        # 各損失項目ごとに個別のフォルダを生成し、結果を保存
