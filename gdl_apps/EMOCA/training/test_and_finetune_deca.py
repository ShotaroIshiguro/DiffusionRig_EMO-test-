...
"""
Emotional DECAフレームワークのモデルをfine-tuning及びトレーニングするためのスクリプト
coarse・detailなどのトレーニングステージに関する設定や、データセットの準備、チェックポイント管理、トレーニング再開を管理
"""
...
# 指定された設定からトレーニングチェックポイントを取得し、その情報を辞書形式で返す
...
# 指定ディレクトリから最新or損失値が最小のチェックポイントを検索
...
def single_stage_deca_pass(deca, cfg, stage, prefix, dm=None, logger=None,
    ...
    # Pytorch_lightning由来のデータモジュールの作成
    if dm is None:
    ...
    # Pytorch_LightningのTrainerインスタンスを作成
    ...
    if stage == "train":
        ...
        if hasattr(cfg.learning, 'checkpoint_after_training'):
            # トレーニング後は必要に応じてベストなチェックポイントを読み込む
...
# DECAモデルをcoarse・detailの2段階に分けてfine-tuningする関数
def finetune_deca(cfg_coarse, cfg_detail, test_first=True, start_i=-1, resume_from_previous = True, force_new_location = False,
    ...
    # データモジュールdm、データセット名sequence_nameを取得
    dm, sequence_name = prepare_data(configs[0])
    dm.setup()

    # get_checkpoint_with_kwargsを使用して、前のステージからチェックポイントを取得
    ...
    # coarseとdetailのトレーニング・テストをループで実行
    deca = None
    if start_i >= 0 or force_new_location:
...
# coarse・detailトレーニングステージの設定を準備し、finetune_deca関数で微調整プロセスを開始
...
def main():
    ...
    if len(sys.argv) >= 5:
        ...
        coarse_override = []
        detail_override = []
    # 以前の設定が読み込まれていれば、finetune_decaを直接呼出し
    if configured:
