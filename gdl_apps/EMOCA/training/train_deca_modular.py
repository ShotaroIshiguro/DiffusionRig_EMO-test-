...
"""
DECAモデルの段階的なトレーニングプロセスを管理
トレーニングとチェックポイントを管理し、実験のログを記録
"""
...
# モデルの過去のチェックポイントを取得し、次のトレーニングに使用できるように準備
...
def train_deca(configs: list, stage_types: list, stage_prefixes: list, stage_names: list, start_i=-1, prepare_data=None, force_new_location=False):
    ...
    # start_iまたはforce_new_locationが指定されている場合、前回のチェックポイントを取得
    ...
    if start_i == -1:
        start_i = 0
    # 各ステージの設定でsingle_stage_deca_passを呼び出すことでモデルをトレーニング
...
def configure(cfg_default, cfg_overrides):
    ...
    return cfg
"""
Hydraでは設定を階層的に管理可能。YAMLファイルを使用して、各モジュールの設定を分離
"""
