...
"""
DECAとEMOCAの設定を読み取り、適切なモデルとデータをロードする
"""

# 設定内のパスを環境に適応するために修正
# relative_to_pathとreplace_root_pathを使用して、現在のディレクトリ構造に合わせたパスに置換
...
def load_deca(conf,
    ...
    print(conf.keys())
    # 指定ステージ coarse or detail の設定を取得
...
# モデルが使用するアセット(FLAME、テクスチャ)のディレクトリを修正
...
# モデルと設定を統合し、必要に応じて異なるステージのモデルをロード
def load_model(path_to_models,
    ...
    # 指定されたステージのモデルをロード、していないときはcoarse
    deca = load_deca(conf,
              stage,
              mode,
              relative_to_path,
