...
"""
DECAベースのシステムにより、顔復元、データ管理、データローディング、トレーニングデータの可視化、
およびメモリ使用量のモニタリングを行うための処理を提供
"""

# FLAMEモデルやテクスチャデータなどのパスを調整
...
def load_data(path_to_models=None,
    ...
    # 実行ディレクトリから設定ファイルcfg.yamlを読み込む
    with open(Path(run_path) / "cfg.yaml", "r") as f:
        # omegaconfにより辞書としてロード
    ...
    dm, name = prepare_data(cfg)    # 設定に基づいてDataModuleを初期化
    dm.setup()                      # データモジュールを初期化し、トレーニング・検証・テストデータセットをロード
    return dm

# データセット内のサンプルを取得し、エンコード・デコードを実行
