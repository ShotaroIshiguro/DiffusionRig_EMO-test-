...
"""
EMOCA or DECAのモデルをパッケージ化するためのスクリプト
特定のモデルディレクトリを入力として受け取り、必要な構成ファイルやアセットを含む新しいディレクトリにモデルをコピーおよび整備
オプションでモデルからテクスチャ情報を削除し、公開可能な形式に変換する機能も提供
"""

# 指定入力ディレクトリからモデルと関連ファイルをコピーし、出力ディレクトリに保存
def package_model(input_dir, output_dir, asset_dir, overwrite=False, remove_bfm_textures=False):
    input_dir = Path(input_dir) 
    output_dir = Path(output_dir)
    asset_dir = Path(asset_dir)     # テクスチャやFLAMEモデル
